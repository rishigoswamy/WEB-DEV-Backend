...
const UsersController = (app) => {
  app.get('/api/login/status', (req, res) => {
    res.json({"status": 'User Controller API is up and running!'})
  });
  ...
  app.put("/api/add-address/", authenticate, async (req, res) => {
    let address = await findOneAddressDao(req.body.address);
    if (address == null) {
      address = await createAddressDao(req.body.address);
    }
    const addAddress = await updateUserDao(address._id, req.body.uid);
  ...
  app.put("/api/remove-address/", authenticate, async (req, res) => {
    const addAddress = await removeUserDao(req.body.aid, req.body.uid);
